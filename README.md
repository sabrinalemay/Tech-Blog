# Tech-Blog
## table of contents
- [Description](#description)
- [License](#license)
- [Installation](#installation)
- [Usage](#usage)
- [Contributing](#contibuting)
- [Tests](#tests)
## Description
Blog where you can make new posts and other developers can comment and share their own posts.
## License
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
## Installation
You will need to git clone and run npm start
## Contributing
Pull requests are welcome.
## Featured
express-handlebars, mySQL2, sequelize, dotenv, bcrypt, express-session, and connect-session-sequelize
## Email
If you would like to contact me with questions, ideas, or ways you would improve the project please click the link below!
  My Email: [b33na1997@gmail.com](mailto:b33na1997@gmail.com)
## Github
Here is my GitHub page: [https://github.com/sabrinalemay/Tech-Blog]
